...
void testInValid2() {
	std::cout << "-- check invalid plugin: version info present, but for wrong product" << std::endl;
	PluginManager man(MODULE_DIR,"",
			DEFAULT_DEBUG_SUFFIX, false,
			PluginManagerInterface::PluginVersionDiscard);
	try {
...
void testInValid3() {
	std::cout << "-- check invalid plugin: version info missmatch " << std::endl;
	PluginManager man(MODULE_DIR,"",
			DEFAULT_DEBUG_SUFFIX, false,
			PluginManagerInterface::PluginVersionDiscard);
	try {
...
void testInValid4() {
	...
	if (logStr.find("contains no valid charon-core version information") == std::string::npos) {
		throw std::runtime_error("missing version not detected");
	}
...
void testInValid5() {
	...
	if (logStr.find("was linked against charon-core version") == std::string::npos) {
		throw std::runtime_error("missing version not detected");
	}
	if (logStr.find("loaded successfully") == std::string::npos) {
		throw std::runtime_error("success message missing");
...
int main() {
	...
	ret |= ExceptionHandler::run(testInValid5);
	return ret;
}
