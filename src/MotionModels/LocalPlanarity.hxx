...
template<class T>
std::set<std::string>& MotionModels::LocalPlanarity<T>::getUnknowns()
{
...
template<class T>
void MotionModels::LocalPlanarity<T>::compute(const int xs, const int ys,
		...
		T& rhs)
{
	if (is3d)
	{
		...
		term["c1"] += (xs - this->x) * (xs - this->x) * dx()(t, xs, ys, zs, v) + (xs - this->x)
				* (ys - this->y) * dy()(t, xs, ys, zs, v) + (xs - this->x) * (zs - this->z)
				* dy()(t, xs, ys, zs, v);
		term["c2"] += (ys - this->y) * (xs - this->x) * dx()(t, xs, ys, zs, v) + (ys - this->y)
				* (ys - this->y) * dy()(t, xs, ys, zs, v) + (ys - this->y) * (zs - this->z)
				* dy()(t, xs, ys, zs, v);
		term["c3"] += (zs - this->z) * (xs - this->x) * dx()(t, xs, ys, zs, v) + (zs - this->z)
				* (ys - this->y) * dy()(t, xs, ys, zs, v) + (zs - this->z) * (zs - this->z)
				* dy()(t, xs, ys, zs, v);
	}
	else
	{
		...
		term["c1"] += (xs - this->x) * (xs - this->x) * dx()(0, xs, ys, t, v) + (xs - this->x)
				* (ys - this->y) * dy()(0, xs, ys, t, v);
		term["c2"] += (xs - this->x) * (ys - this->y) * dx()(0, xs, ys, t, v) + (ys - this->y)
				* (ys - this->y) * dy()(0, xs, ys, t, v);
	}
}
	...
	MotionModel<T>::MotionModel("motionmodels_localplanarity", name), flowfunc()
{
	...
	this->setFlowFunctorParams(0.5, 0.4, 0.3, 0.001, -0.002, 0.0015, 0.003,
			-0.001, -0.0008, 0.0012, 0.0001, 0.0021, 0.0001, -0.00007, 0.00003);
	//			    2d	     2d
}

template<class T>
void MotionModels::LocalPlanarity<T>::setFlowFunctorParams(const float a1,
		const float a2, const float a3, const float b11, const float b12,
		const float b13, const float b21, const float b22, const float b23,
		const float b31, const float b32, const float b33, const float c1,
...
template<class T>
void MotionModels::LocalPlanarity<T>::apply(const Pixel<T> & inPixel,
...
template<class T>
void MotionModels::LocalPlanarity<T>::execute()
{
