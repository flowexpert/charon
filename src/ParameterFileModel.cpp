...
QVariant ParameterFileModel::data(const QModelIndex& ind, int role) const {
    ...
    if ((role == Qt::DisplayRole) || (role == Qt::EditRole)) {
        ...
        if ((row >= 0) && ((uint) row < _keys.size())) {
            ...
            case 1:
                ...
                if(_useMetaInfo &&
                        metaInfo()->isParameter(_keys[row], className)) {
                    std::string typestring;
                    try {
                         typestring =
                            metaInfo()->getType(_keys[row], className);
                    }
    ...
    if ((role == Qt::ToolTipRole) && _useMetaInfo) {
        QString ret = metaInfo()->getDocString(_keys[ind.row()],
            getClass(_keys[ind.row()])).c_str();
        if(!ret.isEmpty())
            return ret;
    }
...
bool ParameterFileModel::setData(const QModelIndex& ind,
    ...
    if ((role == Qt::DisplayRole) || (role == Qt::EditRole)) {
        if ((ind.row() >= 0) && ((uint) ind.row() < _keys.size())) {
            ...
            case 1:
                if (value.canConvert(QVariant::String)) {
                    std::string valueStr = value.toString().toAscii().constData();
                    if (_parameterFile->isSet(_keys[ind.row()])
                            && (valueStr ==
                                _parameterFile->get<std::string>(_keys[ind.row()])))
                        return true; // nothing to do

                    _parameterFile->set<std::string> (_keys[ind.row()], valueStr);
...
void ParameterFileModel::load(const QString& fName) {
    ...
    if(fromDialog.isEmpty())
        fromDialog = QFileDialog::getOpenFileName(0, tr("Open File"), _fileName, tr(
            "ParameterFile (*.*)"));
...
bool ParameterFileModel::prefixValid() const {
    if(_prefix.isEmpty())
        return true;
    return (_parameterFile->getKeyList(
            std::string(_prefix.toAscii().constData())+".").size() > 0);
}
...
std::string ParameterFileModel::getClass (std::string name) const {
    ...
    if (_parameterFile->isSet(name + ".type"))
        className =
            _parameterFile->get<std::string>(name + ".type");
    else
...
std::vector<std::string> ParameterFileModel::_prefixFilter
    ...
    for(listIter = list.begin(); listIter != list.end(); listIter++) {
        if (prefixValid()) {
            ...
            if (listIter->substr(0, length) == pref)
                result.push_back(*listIter);
    }
    return result;
}
...
std::vector<std::string> ParameterFileModel::_paramFilter
    ...
    for(obj = objects.begin(); obj != objects.end(); obj++) {
        const std::vector<std::string>& pars =
            metaInfo()->getParameters(getClass(*obj));
