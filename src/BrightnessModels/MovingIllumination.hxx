...
template <class T>
void BrightnessModels::MovingIllumination<T>::compute(	const int xs, const int ys, const int zs,
...
template <class T>
void BrightnessModels::MovingIllumination<T>::setFunctorParams(float a1, float a2)
{
	this->functor.a1 = a1;
	this->functor.a2 = a2;
}
...
template <class T>
void BrightnessModels::MovingIllumination<T>::Functor::operator ()(cimg_library::CImg<T>& sequence) const
{
	for(unsigned int t=0; t< sequence.depth; t++)
	{
		cimg_forXY(sequence,x,y)
		{
			sequence(x,y,t) *=exp(-(cimg::sqr(x-100-0.4*t)+cimg::sqr(y-100-0.5*t))/10000.0);
		}
	} 
}
