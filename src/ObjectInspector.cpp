...
void ObjectInspector::setModel(ParameterFileModel* newModel) {
    ...
    connect(_model, SIGNAL(prefixChanged(QString)),
        _prefix, SLOT(setText(QString)));
    connect(_prefix, SIGNAL(textEdited(QString)),
        _model, SLOT(setPrefix(QString)));
...
void ObjectInspector::delParam() {
    ...
    if(!rowStack.size()){
        QMessageBox::warning(this, tr("delete failed"),
                tr("Please select an item to delete first."));
        return;
    }
...
void ObjectInspector::clear(){
    QMessageBox mbox(QMessageBox::Question, tr("confirm delete"),
            tr("Do you really want to delete the model content?"));
