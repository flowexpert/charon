...
public:
    ...
    ///                         and in-/outputs
    ParameterFileModel(QString fileName = "", QObject* parent = 0,
                                                        QString metaFile = "");

    /// Default destructor.
    virtual ~ParameterFileModel();

    /// @name model/view interface
    //  @{
    ...
    /// @param  parent          index of parent item
    /// @return                 number of columns (currently always 2)
    ...
    /// @return                 data encapsuled in a QVariant
...
private:
    ...
    std::set<std::string> _collectObjects(const std::vector<std::string>&
        list) const;

    /// filter keys starting with the given prefix
    /// @param list				list to filter
    std::vector<std::string> _prefixFilter(const std::vector<std::string>&
        list) const;
