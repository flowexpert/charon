...
public:
	...
	BrightnessModel(const std::string& classname, const std::string& name = "") : 
		ParameteredObject(classname,name,"computes the vectors for brightness-modeling")
	{
		_addOutputSlot(out,"this","Pointer to itself","BrightnessModel*");
		_addOutputSlot(brightnessFunctor,"brightnessfunctor", "Pointer to BrightnessFunctor of the Model",
