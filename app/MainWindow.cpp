...
MainWindow::MainWindow(QWidget* myParent) :
	...
	connect(selector, SIGNAL(showClassDoc(QString)),
		docGen, SLOT(showClassDoc(QString)));
	connect(selector, SIGNAL(showDocPage(QString)),
		docGen, SLOT(showDocPage(QString)));
	...
	toolbar->addSeparator();
	action = toolbar->addAction(QIcon(":/icons/zoomIn.png"),
		tr("zoom\nin"), this, SLOT(zoomIn()));
	action->setToolTip(tr("enlarge flowchart items"));
	
	action = toolbar->addAction(QIcon(":/icons/zoomOut.png"),
		tr("zoom\nout"), this, SLOT(zoomOut()));
	action->setToolTip(tr("shrink flowchart items"));
	
	action = toolbar->addAction(QIcon(":/icons/zoomFit.png"),
	...
	fileMenu->addAction(QIcon(":/icons/new.png"), tr("&New"),
		this, SLOT(openNew()), QKeySequence(tr("Ctrl+N")));
	fileMenu->addAction(QIcon(":/icons/open.png"), tr("&Open"),
		this, SLOT(open()), QKeySequence(tr("Ctrl+O")));
	fileMenu->addAction(QIcon(":/icons/save.png"), tr("&Save"),
		inspector, SLOT(saveFile()), QKeySequence(tr("Ctrl+S")));
	...
	fileMenu->addAction(QIcon(":/icons/close.png"), tr("&Exit"),
		this, SLOT(close()), QKeySequence(tr("Ctrl+Q")));

	// edit menu
	QMenu* editMenu   = menuBar()->addMenu(tr("&Edit"));
	editMenu->addAction(QIcon(":/icons/revert.png"),
		tr("reset selected parameters"), inspector,
		SLOT(delParam()), QKeySequence(tr("Ctrl+R")));
	editMenu->addAction(QIcon(":/icons/revert_all.png"),
		tr("clear flowchart"), inspector,
		SLOT(clear()), QKeySequence(tr("Ctrl+Shift+R")));

	// view menu
	QMenu* viewMenu   = menuBar()->addMenu(tr("&View"));
	viewMenu->addAction(QIcon(":/icons/zoomIn.png"), tr("zoom in"),
		this, SLOT(zoomIn()), QKeySequence(tr("Ctrl++")));
	viewMenu->addAction(QIcon(":/icons/zoomOut.png"), tr("zoom out"),
		this, SLOT(zoomOut()), QKeySequence(tr("Ctrl+-")));
	viewMenu->addAction(QIcon(":/icons/zoomFit.png"), tr("zoom fit"),
		this, SLOT(zoomFit()), QKeySequence(tr("Ctrl+HOME")));
	...
	helpMenu->addAction(QIcon(":/icons/help.png"), tr("&Help"),
		docGen, SLOT(showHelp()), QKeySequence(tr("F1")));
	helpMenu->addAction(QIcon(":/icons/intro.png"), tr("&Introduction"),
		docGen,	SLOT(showIntro()), QKeySequence(tr("Shift+F1")));
	helpMenu->addAction(appicon, tr("&About ParamEdit"), this,
		SLOT(_showAbout()));
	helpMenu->addAction(QIcon(":/icons/qt.png"), tr("About &Qt"),
		this, SLOT(_showAboutQt()));
...
void MainWindow::_showAbout() const {
    ...
    aboutBox.setText(
        tr( "This is <b>ParamEdit</b> by Jens-Malte Gottfried "
